window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULTS = Object.freeze({
			...
			volume: 0.6, // 0..1
			text: "有新的审批请求，请查看",
			scope: "all", // all（所有会话）| current（仅当前会话）
		...
		// 保留当前正在播放的 <audio> 引用：detached 的 Audio 元素一旦失去引用，
		// 可能被 GC 提前回收，导致自定义提示音被截断或干脆不响。
		 ...
		 * @returns 是否真的能出声（AudioContext 未处于 suspended/closed）。
		 */
		function playChime(volume) {
			const ctx = ensureAudioContext();
			if (!ctx || ctx.state === "closed") {
				console.warn("[approval-voice] 内置提示音不可用：AudioContext 缺失");
				return false;
			}
			...
			// suspended 时振荡器不会发声（浏览器自动播放策略还没解锁）。
			return ctx.state === "running";
		}
		/**
		 * 播放自定义提示音文件（支持 mp3/wav/ogg 等浏览器 `<audio>` 可解码的格式）。
		 * @returns 是否确认起播（`playing` 事件或 `play()` 承诺成功）。
		 */
		function playSoundFile(dataUrl, volume) {
			return new Promise((resolve) => {
				...
				const done = (ok, why) => {
					if (settled) return;
					settled = true;
					if (!ok) console.warn("[approval-voice] 自定义提示音未起播：", why);
					resolve(ok);
				};
				try {
					if (currentAudio) {
					...
					currentAudio = audio;
					audio.volume = Math.max(0, Math.min(1, volume === undefined ? 1 : volume));
					audio.addEventListener("playing", () => done(true), { once: true });
					audio.addEventListener("error", () => done(false, "media error"), { once: true });
					const started = audio.play();
					if (started && typeof started.then === "function") started.then(() => done(true)).catch((error) => done(false, error && error.name ? error.name : error));
					// 既没起播也没报错（例如元素被回收）：1 秒后判定失败，让内置提示音顶上。
					setTimeout(() => {
						if (settled) return;
						try { audio.pause(); } catch { /* ignore */ }
						done(false, "1s 内没有 playing 事件");
					}, 1000);
				} catch (error) {
					done(false, error && error.message ? error.message : error);
				}
			});
		}
		...
		function speak(text) {
			...
			try {
				...
				if (zhVoice === undefined) {
					console.warn("[approval-voice] 未找到中文语音，跳过语音播报");
					return false;
				}
		...
		/**
		 * 按一次提醒实际发声：提示音（自定义优先，失败自动回退内置）+ 语音播报。
		 * @returns 是否至少成功发出了一种声音。
		 */
		function playNow(overrides) {
			const cfg = { ...config, ...(overrides || {}) };
			if (!cfg.enabled || cfg.mode === "off") return Promise.resolve(false);
			const mode = cfg.mode;
			const jobs = [];
			if (mode === "beep" || mode === "both") {
				if (cfg.sound) {
					// 自定义提示音失败（自动播放被拒、解码失败、元素被回收）时回退内置提示音，
					// 否则「仅提示音 + 自定义提示音」会彻底静音。
					jobs.push(playSoundFile(cfg.sound, cfg.volume).then((ok) => ok || playChime(cfg.volume)));
				} else {
					jobs.push(Promise.resolve(playChime(cfg.volume)));
				}
			}
			const spoke = mode === "speech" || mode === "both" ? speak(cfg.text) : false;
			if (jobs.length === 0) return Promise.resolve(spoke);
		...
		function claimBell(eventKey) {
			try {
				...
				if (claim && claim.eventKey === eventKey && typeof claim.ts === "number" && Date.now() - claim.ts < CLAIM_TTL) return false;
		...
		/** 本标签页实际没能出声：让出 claim，并请其它标签页（可能已解锁音频）补响一次。 */
		function releaseBell(eventKey, attempt) {
			try {
				const raw = localStorage.getItem(BELL_KEY);
				if (raw) {
					const claim = JSON.parse(raw);
					if (claim && claim.eventKey === eventKey) localStorage.setItem(BELL_KEY, JSON.stringify({ eventKey: null, ts: 0 }));
				}
			} catch { /* ignore */ }
			if (typeof attempt === "number" && attempt >= 1) return; // 只补响一次，避免标签页互相弹球
			broadcastBell({ type: "bell-failed", eventKey, source: tabId, attempt: (attempt || 0) + 1 });
		}
		/**
		 * 协调一次提醒。聚焦标签页立刻响（delay 0），后台标签页稍等，
		 * 这样若有其它聚焦的 DSH 标签页，它会抢先响；否则由最先到时的后台标签页响。
		 */
		function maybePlay(eventKey, bell, attempt) {
			...
			setTimeout(() => {
				if (!config.enabled) return;
				if (!claimBell(eventKey)) return;
				Promise.resolve(playNow(bell || null)).then((audible) => {
					if (audible === false) releaseBell(eventKey, attempt);
				}).catch(() => { /* ignore */ });
			}, delay);
		}
		/** 收到其它标签页广播的「响铃 / 响铃失败」消息。 */
		function onBellMessage(msg) {
			if (!msg || !msg.eventKey) return;
			if (config.scope !== "all") return; // “仅当前会话”模式忽略跨会话提醒
			if (msg.source === tabId) return; // 忽略自己的广播
			if (msg.type === "bell-failed") {
				// 原播放方没出声：允许本页（若已解锁音频）补响。
				alertedKeys.delete(msg.eventKey);
				maybePlay(msg.eventKey, null, msg.attempt);
				return;
			}
			if (msg.type !== "bell") return;
			maybePlay(msg.eventKey, { mode: msg.mode, text: msg.text, volume: msg.volume }, 0);
		}

		// ---------------------------------------------------------------
		// 检测：所有会话的待处理交互 + 当前视图里的审批/提问卡片
		//   1) 全局源（首选）：ctx.uiSession.pendingInteractions —— DSH 客户端维护的
		//      「所有会话」待处理交互快照（侧边栏「等待审批」就是这个源）。只有它才能
		//      覆盖「没有开在任何标签页里」的会话。
		//   2) DOM 标记（兜底）：当前视图渲染出来的卡片，属性为
		//      data-approval-key / data-question-key / data-plan-review-key。
		//      两条路径统一使用 session 作用域的 eventKey，因此不会重复提醒。
		// ---------------------------------------------------------------
		...
		const PENDING_KINDS = new Set(["approval", "question", "plan-review"]);
		let clientCtx = null;
		let globalWatcherActive = false;

		/** 本标签页当前显示（选中）的会话 id；取不到时返回 undefined。 */
		function currentSessionId() {
			try {
				const snapshot = clientCtx && clientCtx.sessions && clientCtx.sessions.list && clientCtx.sessions.list.getSnapshot
					? clientCtx.sessions.list.getSnapshot()
					: undefined;
		...
		/**
		 * 统一的事件标识：session 作用域（跨标签页可去重），拿不到会话 id 时才退化为标签页作用域。
		 */
		function eventKeyFor(kind, key, sessionId) {
			const sid = sessionId === undefined ? currentSessionId() : sessionId;
			return sid ? `session:${sid}:${kind}:${key}` : `${tabId}:${kind}:${key}`;
		}
		function cardOf(el) {
			for (const [attr, kind] of CARD_ATTRIBUTES) {
				const value = el.getAttribute(attr);
				if (value) return { key: value, kind };
			}
			return null;
		}
		...
		/** 当前标签页的 DOM 检测到一张待处理卡片（兜底路径）。 */
		function onCardDetected(card) {
			if (!config.enabled || config.mode === "off") return;
			notify(eventKeyFor(card.kind, card.key));
		}
		/** 统一提醒入口：本页去重 → 广播（所有会话模式）→ 协调播放。 */
		function notify(eventKey) {
			if (!config.enabled || config.mode === "off") return;
			if (alertedKeys.has(eventKey)) return;
			ensureAudioContext();
			if (config.scope === "current") {
				// 仅当前会话：只在本地响，不广播（其它标签页各看各的）。
				alertedKeys.add(eventKey);
				Promise.resolve(playNow()).catch(() => { /* ignore */ });
				return;
			}
			// 所有会话：交给 maybePlay 去重并协调（它会先写入 alertedKeys）。
			const bell = {
				type: "bell",
				eventKey,
				source: tabId,
		...
		/**
		 * 订阅全局待处理交互源。返回取消订阅函数；源不可用时返回 null（调用方退回 DOM 监听）。
		 */
		function startGlobalWatcher(ctx) {
			const source = ctx && ctx.uiSession ? ctx.uiSession.pendingInteractions : undefined;
			if (!source || typeof source.getSnapshot !== "function" || typeof source.subscribe !== "function") {
				console.warn("[approval-voice] 取不到 uiSession.pendingInteractions，退回 DOM 卡片监听（只能覆盖当前视图的会话）");
				return null;
			}
			const seen = new Set();
			let stopped = false;
			const evaluate = () => {
				if (stopped) return;
				let snapshot;
				try { snapshot = source.getSnapshot(); } catch { return; }
				if (!snapshot || typeof snapshot.forEach !== "function") return;
				const live = new Set();
				snapshot.forEach((interaction, sessionId) => {
					if (!interaction || !PENDING_KINDS.has(interaction.kind)) return;
					const key = interaction.key === undefined ? "" : String(interaction.key);
					const eventKey = `session:${sessionId}:${interaction.kind}:${key}`;
					live.add(eventKey);
					if (seen.has(eventKey)) return;
					seen.add(eventKey);
					// 仅当前会话模式：只为本标签页正在显示的那个会话响。
					if (config.scope === "current" && sessionId !== currentSessionId()) return;
					notify(eventKey);
				});
			...
			globalWatcherActive = true;
			evaluate();
			return () => {
				stopped = true;
				globalWatcherActive = false;
		...
		// 插件主体
		// ---------------------------------------------------------------
		const inject = ["slots", "locale", "uiSession", "sessions"];
		function apply(ctx) {
			clientCtx = ctx;
			...
			ctx.effect(() => {
				...
				// 首选：全局待处理交互源（覆盖所有会话，含没开在任何标签页里的会话）。
				const stopGlobalWatcher = startGlobalWatcher(ctx);
				// 兜底：DOM 卡片监听（全局源不可用时）；两条路径 eventKey 一致，不会重复响。
				...
				return () => {
					observer.disconnect();
					if (stopGlobalWatcher) stopGlobalWatcher();
			...
			// 控制台调试接口：window.__approvalVoice.get() / set(...) / test() / pending() / tabId
			try {
				window.__approvalVoice = {
					...
					/** 全局待处理交互快照（所有会话，不只当前视图）。 */
					pending: () => {
						try {
							const snapshot = clientCtx && clientCtx.uiSession ? clientCtx.uiSession.pendingInteractions.getSnapshot() : undefined;
							if (!snapshot || typeof snapshot.forEach !== "function") return [];
							const list = [];
							snapshot.forEach((interaction, sessionId) => {
								if (interaction) list.push({ sessionId, kind: interaction.kind, key: interaction.key });
							});
							return list;
						} catch {
							return [];
					...
					/** 是否已挂上全局源（true = 能覆盖没开在标签页里的会话）。 */
					global: () => globalWatcherActive,
